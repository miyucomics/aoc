...
actives = {y * 1j + x for x in range(world_width) for y in range(world_height) if get(y * 1j + x)}
...
while True:
    removable = [
        active
        for active in actives
        if neighbor_map[int(active.imag * world_width + active.real)] < 4
    ]
    if len(removable) == 0:
        break
    for el in removable:
